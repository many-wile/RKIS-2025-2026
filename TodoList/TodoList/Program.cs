...
namespace TodoList
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			_apiStorage = new ApiDataStorage();
			...
			while (true)
			{
				...
				try
				{
					...
					if (command != null)
					{
						...
						if (command is IUndo && command is not UndoCommand && command is not RedoCommand)
						{
		...
		private static ICommand ParseCommand(string input)
		{
			...
			switch (commandName)
			{
				...
				case "delete":
					if (int.TryParse(args, out int delIndex))
					{
				...
				case "sync":
					return new SyncCommand(args, _storage, _apiStorage);
				case "exit":
					Environment.Exit(0);
					return null;
				default:
